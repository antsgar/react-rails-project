...
class Api::QuestionControllerTest < ActionDispatch::IntegrationTest
  EXAMPLE_QUESTIONS = [{
    ...
    answer: "Focus on delivering value and solving a real problem for your customers."
  }, {
  ...
  test "should return random answer from database" do
    ...
    json_response = @response.parsed_body
    assert_includes [EXAMPLE_QUESTIONS[0][:content], EXAMPLE_QUESTIONS[1][:content]], json_response["content"]
    assert_includes [EXAMPLE_QUESTIONS[0][:answer], EXAMPLE_QUESTIONS[1][:answer]], json_response["answer"]
  end
end
